...
export default class Setting extends Component {
    ...
    addTeamMembers=()=>{
        if(this.currentValue.value!==undefined&&!this.teamIdList.includes(this.currentValue.value)){
            ...
            fetch("http://localhost:5000/insertTeamMember",{
                ...
                if(result.status==200){
                    ...
                    memberList.push({"empName":this.currentValue.label,"empId":this.currentValue.value});
    ...
    addNewProject=()=>{
        ...
        let form = new FormData()
        form.append("row",JSON.stringify({project_id:this.projectId,project_name:this.projectName}));
    ...
    addNewMembers=()=>{
        ...
        if(!this.projectEmpIdList.includes(id)&&id!==""&&name!==""){
            ...
            form.append("data",JSON.stringify({"empName":this.empName,"empId":this.empId,"projectId":project,"positionCode":1}))
            fetch("http://localhost:5000/insertNewMember",{
              method:"post",
              body:form
              }).then((result)=>{
                if(result.status===200){
                    projectMember.push({"empName":name,"empId":id,"projectId":project,"positionCode":1})
    ...
    getTeamMemberPart=()=>{
        return(
            this.state.teamMembers.map((tag,key)=>{
                this.teamIdList.push(tag.empId)
                return<div className="team-member" id={tag.empId}><span>{tag.empName+" "+tag.empId}</span><span className="team-member-delete" onClick={()=>{this.deleteTeamMember(key)}}>X</span></div>
            })
            )
    }
    ...
    getProjectMemberPart=()=>{
        return(
            this.state.projectMember.map((tag,key)=>{
                this.projectEmpIdList.push(tag.empId);
                return<div className="team-member" id={tag.empId}><span>{tag.empName+" "+tag.empId+" "+tag.projectId}</span><span className="team-member-delete" onClick={()=>{this.deleteProjectMember(key)}}>X</span></div>
            })
            )
    }
    ...
    screenTitle=()=>{
        return(["123","1234","12345","123456"].map((value)=>{
           ...
           return( <div className="screen-access"><div className="screen-access-label">{value}</div><input className="screen-access-input" type="number"></input></div>)
        }))
    }
    ...
    getProjectPart=()=>{
        ...
        return( this.state.projects.map((tag,key)=>{
            ...
            return<div className="team-member" id={tag.project_id}><span>{tag.project_name+" "+tag.project_id}</span><span className="team-member-delete" onClick={()=>{this.deleteProject(key)}}>X</span></div>
        }))
    }
    ...
    render(){
        return(<div class="body" >
            ...
            <Tabs style={{width:"auto",position:"absolute"}} activeTab={this.state.activeTab} onChange={(tabId) => this.setState({ activeTab: tabId })} ripple>
                    <Tab>Add members</Tab>
            ...
            <div>
            {this.getTeamMemberPart()}
            </div>
            <ReactAutoCompleteComponent ref={this.autoComplete}
            marginRight="20px" 
            width="200px" 
            id="001" 
            onChange={(value,label)=>{this.currentValue={"value":value,"label":label}} }
            data={this.options} 
            disabled={this.props.disableInput} 
            label=""></ReactAutoCompleteComponent>
            <Button style={{"marginBottom":"8px"}}primary="true" raised ripple onClick={this.addTeamMembers} >Add</Button>
            </section>:<></>}
            {this.state.activeTab==2?<section style={{}}>
                <div>{this.getProjectMemberPart()}</div>
                <div style={{display: "inline-flex"}}><div className="addNewMember"><Textfield
                        onChange={(event) => {this.empName=event.currentTarget.value}}
                        label="Employee Name"
                        style={{width: '200px'}}
                    /></div><div ref="empId" className="addNewMember" ><Textfield
                        onChange={(event) => {this.empId=event.currentTarget.value}}
                        label="Employee Id"
                        style={{width: '200px'}}
                /></div><div className="addNewMember" ><Textfield
                onChange={() => {}}
                label="Position"
                type="number"
                style={{width: '200px'}}
            /></div><Button style={{marginTop:"12px"}}primary="true" raised ripple onClick={this.addNewMembers} >Add</Button>
                {this.state.errorMessage!==""? <div className="errorMessage"style={{color:"red"}}>{this.state.errorMessage}</div>:<></>}
                </div>
            </section>:<></>}
            {this.state.activeTab==1?<section style={{}}>
                ...
                <div><div className="addNewMember"><Textfield
                        ...
                        style={{width: '200px'}}
                    /></div><div ref="empId" className="addNewMember" ><Textfield
                        onChange={(event) => {this.projectId=event.currentTarget.value}}
                        label="Project Id"
                        style={{width: '200px'}}
                /></div><Button primary="true" style={{"marginBottom":"8px"}} raised ripple onClick={this.addNewProject} >Add</Button>
                {this.state.errorMessage!==""? <div className="errorMessage"style={{color:"red"}}>{this.state.errorMessage}</div>:<></>}
                </div>
            </section>:<></>}
