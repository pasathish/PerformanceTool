...
export default class ReactAutoCompleteComponent extends Component {
    ...
    closePopper = () => {
        this.closeSuggestionTimeOut=setTimeout(()=>{this.setState({
            open: false,
        });},200);
    };
    ...
    setValue = (selectValue,label) =>{
        ...
        this.setState({ value: selectValue,open:false,"selectedLabel":selectedLabel });
    ...
    formSuggestionList=(list)=>{
        return list.map((value,key)=>{return (<div tabIndex="0" title={value.label+" "+value.value} className="suggestion" onKeyPress={(event)=>{this.setValueKeyEvent(event,value.value,value.label)}} onClick={()=>{this.setValue(value.value,value.label )}} key={key} dummyKey={value.value}>{value.label +" "+value.value}</div>)});   
    }

    filterSugestion=(value)=>{
        const filterList = this.state.totalSuggestions.filter(suggestion=>(suggestion.value+" "+suggestion.label).toLowerCase().includes(value.toLowerCase()))
