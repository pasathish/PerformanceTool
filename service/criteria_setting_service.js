...
const criteria_add_service=require('../service/criteria_add_service');
const addService=new criteria_add_service();
class CriteriaService{
    
   getAll(res,project_id){
    return new Promise((resolve,reject)=>{  addService.getAll(null).then((result)=>{
                ...
                dao.getAll(project_id).then((settingList)=>{
                    ...
                    settingList=settingList.filter(element => {
                        if(criteriaMap.get(element.criteria_id)){
                            element.criteria_name=criteriaMap.get(element.criteria_id).criteria_name;
                            element.criteria_description=criteriaMap.get(element.criteria_id).criteria_description;
                    ...
                    Array.from(criteriaMap.keys()).forEach((criteriaId)=>{
                         ...
                         settingList.push(this.getDefaultSetting(criteriaMap.get(criteriaId)));
                    })
                    if(res === null){
                        resolve(settingList);
